...
func HandleReturn(v iface.VMInterface, op bytecode.Opcode, ins bytecode.Instructions) (any, error) {
	...
	var returnValue any
	if frame.ToCount > 0 {
		if sp > frame.BasePointer+frame.ToCount {
			returnValue = v.Pop()
		}
	} else if sp > frame.BasePointer {
		returnValue = v.Pop()
	}

	if frame.ToCount > 0 {
		stack := v.GetStack()
		toStart := frame.BasePointer
		toEnd := toStart + frame.ToCount
		if toStart >= 0 && toEnd <= len(stack) {
			toFn, ok := stack[toStart].(*bytecode.CompiledFunction)
			if ok {
				v.SetSP(toStart)
				for i := toStart + 1; i < toEnd; i++ {
					if nextTo, nextOk := stack[i].(*bytecode.CompiledFunction); nextOk {
						v.Push(nextTo)
					}
				}
				locals := v.GetLocals()
				...
				newFrame := &iface.Frame{
					...
					Ip:          0,
					BasePointer: toStart,
					ToCount:     frame.ToCount - 1,
				}
				frames := v.GetFrames()
				frames[v.GetFrameIndex()] = newFrame
				v.SetFrames(frames)
